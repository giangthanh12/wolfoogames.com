...
<div class="container-fluid">
    <div class="row">
        <div class="col-sm-6">
            <h1><a style="font-size: 24px; color: #555555; display: inline-block; margin-right: 15px;" data-widget="pushmenu" href="#" role="button"><i class="fas fa-bars"></i></a> Edit Charactor</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="/dashboard">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="/content/game">Charactor</a></li>
                <li class="breadcrumb-item active"><strong>Edit charactor</strong></li>
            </ol>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
            <form class="form-coltrol" method="post" action="/content/charactor/save" enctype="multipart/form-data">@csrf
            <div class="card">
                <div class="card-body">
                @if (is_null($charactor) || empty($charactor))
                    <div style="text-align: center;">Item not found!</div>
                @else
                    @php
                    $post_id        = $charactor->id;

                    $title          = App\Utils::get_array_item_value($selected_lang, json_decode($charactor->title, true), '');
                    $short_desc     = App\Utils::get_array_item_value($selected_lang, json_decode($charactor->short_desc, true), '');
                    $content        = App\Utils::get_array_item_value($selected_lang, json_decode($charactor->content, true), '');
                    $images         = json_decode($charactor->images, true);
                    $icon_image     = App\Utils::get_array_item_value('icon', $images, '');

                    $post_seo = $charactor->post_seo;
                    ...
                    <div class="row">
                        <div class="col-sm-9 col-12">
                            ...
                            <div class="input-group mb-3">
                                <input required type="text" class="form-control" id="title" name="title" placeholder="Charactor name" value="{{ old('title', $title) }}" />
                            </div>
                        ...
                        <div class="col-sm-3 col-12">
                            <div class="input-group mb-3">
                                <div style="text-align: center;width: 100%">Charactor image</div>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#language').on('change', function () {
                location.href = '/content/charactor/edit?id={{ $post_id }}&language=' + $('#language').val();
            });

            $('#btn-reset').on('click', function() {
                location.href = '/content/charactor';
            });
